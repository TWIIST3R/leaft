...
export function TalentMarketOffersCard({
  ...
  return (
    <div className="rounded-3xl border border-[#c8d4c8] bg-[#fbfcfb] p-4 shadow-sm sm:p-6">
      <p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand)]">Comparatif marché (offres d&apos;emploi)</p>
      <p className="mt-1 text-sm text-[color:rgba(11,11,11,0.6)]">
        Estimation à partir des annonces publiées sur <strong>Indeed</strong> et <strong>Glassdoor</strong> en France, pour des intitulés proches du vôtre.
      </p>
      {!hasdataConfigured && (
      ...
      {m?.search_keyword && (
        <p className="mt-2 text-xs text-[color:rgba(11,11,11,0.5)]">
          Poste de référence : « {m.search_keyword} » — France
