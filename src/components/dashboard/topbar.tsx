...
export function DashboardTopbar({ mode = "rh" }: DashboardTopbarProps) {
  const isTalent = mode === "talent";
  return (
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        <p className="text-xs uppercase tracking-wide text-[color:rgba(11,11,11,0.45)]">{isTalent ? "Espace Talent" : "Espace RH"}</p>
        <h1 className="text-lg font-semibold text-[var(--text)]">
          {isTalent ? "Mon espace collaborateur" : "Pilotage des talents & rémunérations"}
        </h1>
      </div>
